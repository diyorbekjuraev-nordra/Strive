...
interface EmojiPickerProps
  extends React.ComponentProps<typeof EmojiPickerPrimitive.Root> {
  layer?: LAYER_VARIANTS;
}

const EmojiPicker = memo(
  ({ className, layer = 1, ...props }: EmojiPickerProps) => {
    return (
...
interface IEmojiPickerListCategoryHeaderProps
  extends EmojiPickerListCategoryHeaderProps {
  layer?: LAYER_VARIANTS;
}
...
function EmojiPickerCategoryHeader({
  ...
  layer = 1,
  ...props
}: IEmojiPickerListCategoryHeaderProps) {
  return (
    <div
      {...props}
      className={cn(
        "text-muted-foreground px-3 pb-2 pt-3.5 text-xs leading-none",
        LAYER_CLASSNAMES[layer]
      )}
